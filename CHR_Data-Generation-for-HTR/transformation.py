...
def flag_judge(list_child_already):
    ...
    if not list_child_already[0][0]:
        if not list_child_already[0][1]:
            flag_situation_judge = 1  # Corner 1 is empty, corner 2 is also empty.
        else:
            flag_situation_judge = 2  # Corner point 1 is empty, corner point 2 is not empty.
    elif not list_child_already[0][1]:
        flag_situation_judge = 3  # Corner 1 is not empty, corner 2 is empty.
    else:
        flag_situation_judge = 4  # Corner point 1 is not empty, and corner point 2 is not empty either.
...
# Method 1: Bezier curve deformation
def bezier_transformation(list_already, list_information, flag_special):
    ...
    """
    ratio = 0 #default 0.1
    ratio_change = 0.5 # default 0.5
    # Take the data
    list_bezier_inf = list_information[2]  # Take the part of the entire information used for Bessel transformation.
    list_total = []
    for k in range(len(list_bezier_inf)):
        list_total.append([[[], []], []])
    # First, mark the corners of list_total according to the different situations of list_already.
    if flag_special == 2:  # Corner point 1 is empty, corner point 2 is not empty.
        list_total[-1][0][1] = list_already[0][1]
    elif flag_special == 3:  #  Corner 1 is not empty, corner 2 is empty.
        list_total[0][0][0] = list_already[0][0]
    elif flag_special == 4:  # Corner point 1 is not empty, and corner point 2 is not empty either.
        list_total[-1][0][1] = list_already[0][1]
        list_total[0][0][0] = list_already[0][0]

    # Use list_total as the main loop, calculate the data and enter it, calculate list_total
    for index_list_total in range(len(list_total)):
        ...
        # First, process the first corner point.
        if not list_total[index_list_total][0][0]:  # Indicates that vertex 1 is empty; only empty vertices are processed.
            # Indicates that vertex 2 is also empty. If both are empty, it means that they are the first pair in the entire image, so the landing point is determined directly based on itself.
            if not list_total[index_list_total][0][1]:
                x_get, y_get = list_bezier[0][0][0], list_bezier[0][0][1]
                x_already = random.uniform(x_get - list_bezier[2][0][0], x_get + list_bezier[2][0][0] + ratio)
                y_already = random.uniform(y_get - list_bezier[2][0][1], y_get + list_bezier[2][0][1] + ratio)
                pt_already = [x_already, y_already]
                list_total[index_list_total][0][0].extend(pt_already)
            # Indicates that vertex 2 exists, so vertex 2 should be used as the reference point.
            else:
                ...
                pt_already = [x_already, y_already]
                list_total[index_list_total][0][0].extend(pt_already)
        # Reprocess the second corner point
        if not list_total[index_list_total][0][1]:  # Indicates that vertex 2 is empty, and only empty vertices are processed. At this point, vertex 1 must exist, so vertex 1 is used as the reference point to determine the landing point.
            ...
            x_already = random.uniform(x_get - list_bezier[2][1][0], x_get + list_bezier[2][1][0] + ratio)
            y_already = random.uniform(y_get - list_bezier[2][1][1], y_get + list_bezier[2][1][1] + ratio)
            pt_already = [x_already, y_already]
            list_total[index_list_total][0][1].extend(pt_already)
            # If there is another pair, this point needs to be added to the first point.
            if index_list_total + 1 < len(list_total):
                list_total[index_list_total + 1][0][0].extend(pt_already)
        # After obtaining two corner points, calculate the third Bézier control point.
        ...
        x_change = float(ratio_change * x_corner1_change) + float(ratio_change * x_corner2_change)
        y_change = float(ratio_change * y_corner1_change) + float(ratio_change * y_corner2_change)
        x_get, y_get = list_bezier[1][0][0] + x_change, list_bezier[1][0][1] + y_change
        x_already = random.uniform(x_get - list_bezier[3][0], x_get + list_bezier[3][0] + ratio)
        y_already = random.uniform(y_get - list_bezier[3][0], y_get + list_bezier[3][0] + ratio)
        pt_already = [x_already, y_already]
    ...
    for list_total_child in list_total:
        ...
        x, y = points[:, 0].astype(np.int_).tolist(), points[:, 1].astype(np.int_).tolist()
        np.int32
        # 一个个点打进list_ske
